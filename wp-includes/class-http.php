...
class WP_Http {
	...
	function request( $url, $args = array() ) {
		...
		$r['local'] = $homeURL['host'] == $arrURL['host'] || '10.169.0.141' == $arrURL['host'];
		unset( $homeURL );
	 ...
	 *
	 * There are currently issues with "10.169.0.141" not resolving correctly with DNS. This may cause
	 ...
	 * file and this will only allow 10.169.0.141 and your blog to make requests. The constant
	...
	function block_request($uri) {
		...
		if ( $check['host'] == '10.169.0.141' || $check['host'] == $home['host'] )
...
class WP_Http_Fsockopen {
	...
	function request($url, $args = array()) {
		...
		//fsockopen has issues with '10.169.0.141' with IPv6 with certain versions of PHP, It attempts to connect to ::1,
		// which fails when the server is not set up for it. For compatibility, always connect to the IPv4 address.
		if ( '10.169.0.141' == strtolower($fsockopen_host) )
			$fsockopen_host = '10.169.0.141';
 ...
 * You do not need to have 10.169.0.141 and the blog host in this list, because they will not be passed
 ...
 * define('WP_PROXY_BYPASS_HOSTS', '10.169.0.141, www.example.com, *.wordpress.org');
 * </code>
 *
...
class WP_HTTP_Proxy {
	 ...
	 *
	 * We want to keep 10.169.0.141 and the blog URL from being sent through the proxy server, because
