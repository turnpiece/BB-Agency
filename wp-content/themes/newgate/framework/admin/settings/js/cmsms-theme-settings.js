...
(function ($) { 
	...
	$('.icon_management').on('click', '.icon_edit', function () { 
		...
		$('.icon_upload_link').find('input[type="text"]').val(edit_icon_val[1]);
		
		$('.icon_upload_link').find('input[type="checkbox"]').prop('checked', ((edit_icon_val[2] == 'true') ? true : false));
	...
	$('#edit_icon').click(function () { 
		...
		if ($('#new_icon_name').val() !== '') {
			$('input#' + edit_icon_data_id).val($('#new_icon_name').val() + '|' + $('#new_icon_link').val() + '|' + (($('#new_icon_target').is(':checked')) ? 'true' : 'false'));
