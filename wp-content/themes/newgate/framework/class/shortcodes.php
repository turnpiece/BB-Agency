...
function cmsmasters_featured_block($atts, $content = null) {
    ...
    $out = '<div class="featured_block" style="background-color:' . $bgcolor . ';">' . 
	...
	if ($button != '') {
        $out .= '<div class="colored_button"><a class="button_medium" href="' . $buttonlink . '" title="' . $buttontext . '"';
...
function cmsmasters_person_block($atts, $content = null) {
	...
	$out .= '<h4 class="person_title">' . $title . '</h4>';
	...
	if ($content != '') {
		$out .= '<div class="divider person_divider"></div>' . 
...
function cmsmasters_stats_bar($atts, $content = null) {
	...
	return '<div class="percent_item">' . 
		'<div class="percent_item_colored_wrap" style="width:' . $value . '%;">' . 
			'<div class="percent_item_colored"></div>' . 
        ...
        while ($posttype_query->have_posts()) : $posttype_query->the_post();
			...
			if ($post_type != 'testimonial') {
				if ($show_images == 'true') {
					...
					if ($type == 'slider' || $type == 'album' || $type == 'gallery') {
						if (has_post_thumbnail()) {
							...
							$out .= '<figure>' . 
								wp_get_attachment_image($img_number_list[0], 'post-thumbnail', false, array( 
									'class' => 'fullwidth', 
									'alt' => cmsms_title(get_the_ID(), false), 
									'title' => cmsms_title(get_the_ID(), false) 
								)) . 
					...
					$out .= '<div class="portfolio_rollover">';
						...
						} else if ($type == 'video' && ($cmsms_project_video_link != '' || $cmsms_project_video_links != '' || $cmsms_post_video_link != '' || $cmsms_post_video_links != '')) {
							if ($post_type == 'project') {
						...
						foreach ($img_number_list as $img_number_list_image) {
							...
							$out .= '<figure>' . 
								'<a href="' . $link_href[0] . '" data-group="img_' . get_the_ID() . '" title="' . cmsms_title(get_the_ID(), false) . '" class="preloader highImg jackbox">' . 
									wp_get_attachment_image($img_number_list_image, 'project-thumb-half', false, array( 
										'class' => 'fullwidth', 
										'alt' => cmsms_title(get_the_ID(), false), 
										'title' => cmsms_title(get_the_ID(), false) 
									)) . 
				...
				$out .= '<header class="entry-header">' . 
					...
					'</h4>';
					if ($show_info == 'true') {
						$out .= '<abbr class="published" title="' . get_the_date() . '">' . 
							get_the_date() . 
						'</abbr>';
					}
				$out .= '</header>';
