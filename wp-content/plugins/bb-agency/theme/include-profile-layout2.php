...
?><script type="text/javascript">
			function addtofv(ids){
					jQuery.ajax({type: 'POST',url: '<?php echo get_bloginfo('url') ?>/wp-admin/admin-ajax.php',
									 data: {action: 'bb_agency_save_favorite',  talentID: ids},
										...
										if(results=='error'){ 
											...
											alert("You're not signed in");
												 ...
												 if(jQuery("#save_fav_li").text() == 'Add to Favorites'){
													jQuery("#save_fav_li").text('<?php echo __("Remove from Favorites", bb_agency_TEXTDOMAIN); ?>');
												 } else {
													jQuery("#save_fav_li").text('<?php echo __("Add to Favorites", bb_agency_TEXTDOMAIN); ?>');
											    }
										}
									}
					   }); // ajax submit
