...
<?php if(!empty($bb_agency_option_agencylogo)){ ?>
  <img src="<?php echo $bb_agency_option_agencylogo; ?>" title="<?php echo $bb_agency_option_agencyname; ?>" />
<?php }else{ ?>
<?php echo $bb_agency_option_agencyname; ?>
<?php } ?>
</div>
 <br clear="all" />  */?>
    ...
    if($_POST['print_type']=="print-polaroids"){$printType="Polaroid";}
   	...
   	$queryImg = "SELECT * FROM ". table_agency_profile_media ." media WHERE ProfileID =  \"". $ProfileID ."\" AND ProfileMediaType = \"".$printType."\" ORDER BY $orderBy";
		...
		//	 echo "<img $left  $widthAndHeight id='".$dataImg["ProfileMediaID"]."' src=\"". bb_agency_UPLOADDIR . $ProfileGallery ."/". $dataImg['ProfileMediaURL'] ."\" alt='' class='allimages_thumbs' />";
					...
					echo "<img id='".$dataImg["ProfileMediaID"]."' src=\"".get_bloginfo("url")."/wp-content/plugins/bb-agency/tasks/timthumb.php?src=". bb_agency_BASEREL . $ProfileGallery ."/". $dataImg['ProfileMediaURL'] .$timthumbHW."\" alt='' class='allimages_thumbs' />";
					
				if($rowCount==$showFooter){
				   $rowCount=0; //reset to loop another row 
				  echo '<br clear="all"><img style="width:347px; '.$logoMarginTop.'" src="'.get_bloginfo("url").'/wp-content/plugins/bb-agency/theme/custom-layout6/images/address.jpg"><br clear="all">';
...
if($rowCount!=$showFooter AND $rowCount!="0"){
 echo '<br clear="all"><img style="width:347px;" src="'.get_bloginfo("url").'/wp-content/plugins/bb-agency/theme/custom-layout6/images/address.jpg"><br clear="all">';
}
?>
</div>
</body>
