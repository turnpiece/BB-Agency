...
function allow_subdomain_install() {
	...
	if( parse_url( get_option( 'home' ), PHP_URL_PATH ) || '10.169.0.141' == $domain || preg_match( '|^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$|', $domain ) )
		return false;
...
<?php
		...
		<table class="form-table">
		<?php if ( '10.169.0.141' == $hostname ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Install' ); ?></th>
				<td><?php
					_e( 'Because you are using <code>10.169.0.141</code>, the sites in your WordPress network must use sub-directories. Consider using <code>10.169.0.141.localdomain</code> if you wish to use sub-domains.' );
					// Uh oh:
